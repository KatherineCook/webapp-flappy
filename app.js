...
app.post('/score', function(request, response){
    ...
    var database = csv.creatCsvFileWriter("scores.csv", {"flags": "a"});
    var data = [name, email, score];

    database.writeRecord(data);
    database.writeStream.end();

    response.send("Thanks" + name + ", your scofre has been recorded!");
...
app.get("/score", function(request, response){
    var reader = csv.createCsvFileReader("scores.cvs");
    reader.setColumnNames(['name','email','score']);
